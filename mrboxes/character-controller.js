...
export class CharacterController{
 ...
 advance(dt,keys){
  ...
  if(!direction)this.continuation=null;
  if(!this.turn&&direction){
   const firstFoot=this.continuation?.direction===direction?this.continuation.foot:direction===1?1:0;
  ...
  if(this.turn){
   ...
   if(u===1){
    if(t.step===2||(!this.completeTurnOnRelease&&direction!==t.direction)){
     if(direction===t.direction)this.continuation={direction,foot:1-event.foot};
