...
export function poseCharacterFeet(m,feet){for(let i=0;i<2;i++)solveLeg(m,i,feet[i]);}
...
export function updateCharacterTurn(m,dt,foot,u,distance,serial){
 ...
 m.pelvis.position.set(spring(m,'turnX',-side*.018*envelope*a,dt,95,19),spring(m,'turnY',.560-.028*a+.009*envelope*a,dt,100,20),spring(m,'turnZ',.006*a,dt,95,19));
